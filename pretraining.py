...
parser.add_argument('--hico-k', default=16384, type=int,
...
def main():
    ...
    model = stda_net.builder.STDA_Net(opts.encoder_args,args.hico_dim,args.hico_k,args.hico_m,args.hico_t)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        loss,acc1= train(train_loader, model, criterion1, optimizer, epoch, args)
...
def train(train_loader, model, criterion1, optimizer, epoch, args):
    ...
    for i, (xq, xk) in enumerate(train_loader):
        # measure data loading time
        data_time.update(time.time() - end)

        xq = xq.float().cuda(non_blocking=True)
        xk = xk.float().cuda(non_blocking=True)
        
        # compute output
        output1, output2, output3, output4, output5, output6, target1, target2, target3, target4, target5, target6 = \
            model(xq, xk)

        batch_size = output1.size(0)
        
        # intra level
        loss1 = criterion1(output1, target1) + criterion1(output2, target2) + criterion1(output3, target3) + criterion1(output4, target4)
        # inter level
        loss2 = criterion1(output5, target5) + criterion1(output6, target6)
