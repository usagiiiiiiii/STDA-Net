...
class Transformer_block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=1., mlp_out_ratio=1.,
        ...
        self.attn_t = Attention(dim, dim, num_heads=num_heads, qkv_bias=qkv_bias,
                              attn_drop=attn_drop, proj_drop=drop)
        self.attn_s = Attention(dim, int(dim * 1.5),  num_heads=num_heads, qkv_bias=qkv_bias,
                              attn_drop=attn_drop, proj_drop=drop)
        self.attn_g = Attention(dim, dim, num_heads=num_heads, qkv_bias=qkv_bias,
                              attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x, symbol):
        if symbol == 'temporal':
            x = x + self.attn_t(self.norm1(x))
        elif symbol == 'spatial':
            x = x + self.attn_s(self.norm1(x))
        else:
...
class STDA_Encoder(nn.Module):
    ...
    def __init__(self, t_input_size, s_input_size, hidden_size, num_head) -> None:
        ...
        # multi-attention encoders
        self.shared_encoder = Transformer_block(hidden_size, num_head)
    ...
    def forward(self, x):
        ...
        xf = self.fusion(xt, xs)  # global domain

        vt = self.shared_encoder(xt, 'temporal')
        vf = self.shared_encoder(xf, 'global')
        vs = self.shared_encoder(xs, 'spatial')
...
class PretrainingEncoder(nn.Module):
    ...
    def __init__(self, t_input_size, s_input_size, hidden_size, num_head, num_class=60):
        ...
        self.stda_encoder = STDA_Encoder(t_input_size, s_input_size, hidden_size, num_head)

        # temporal domain feature projector
        ...
        # spatial domain feature projector
...
class DownstreamEncoder(nn.Module):
    ...
    def __init__(self, t_input_size, s_input_size, hidden_size, num_head, num_class=60):
        ...
        # linear classifier
        self.fc = nn.Linear(3 * self.d_model, num_class)

    def forward(self, x, knn_eval=False):

        vt, vs, vf = self.stda_encoder(x)

        v = torch.cat([vt, vs, vf], dim=1)
